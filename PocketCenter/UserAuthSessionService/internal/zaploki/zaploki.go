...
type ZapLoki interface {
	...
	Stop()
	WithCreateLogger(zap.Config, string) (*zap.Logger, error)
}
...
func (lp *lokiPusher) WithCreateLogger(cfg zap.Config, sinkKey string) (*zap.Logger, error) {
	err := zap.RegisterSink(sinkKey, lp.Sink)
	if err != nil {
		log.Fatal(err)
	}

	fullSinkKey := fmt.Sprintf("%s://", sinkKey)
